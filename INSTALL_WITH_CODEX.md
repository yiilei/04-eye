...
```text
请安装并配置“采光”。先检查我的 Mac 芯片，安装 GitHub Release 中对应的应用；如需要修复或重新打包，再取得与应用版本号一致的 GitHub 源码 tag。引导我在应用中完成小红书登录，并检测 Eagle 是否启动。完成后，帮我抓取最新的小红书创作服务活动。
```
...
5. 优先让用户在应用中主动点击“使用 Chrome 登录”：只读复制当前登录快照，不修改 Chrome Cookie。该方式失败时，再执行 `pnpm xhs:login` 建立只属于采光的隔离扫码会话。
