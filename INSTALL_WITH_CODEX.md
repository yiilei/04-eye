...
```text
请安装并配置“采光”。先检查我的 Mac 芯片，安装 GitHub Release 中对应的应用和源码依赖；运行 `pnpm xhs:login`，引导我使用小红书 App 扫码建立采光独立会话，不读取或修改 Chrome，并检测 Eagle 是否启动。完成后，帮我抓取最新的小红书创作服务活动。
```
...
5. 执行 `pnpm xhs:login`，请用户使用小红书 App 扫描终端二维码，建立只属于采光的隔离会话。命令不得读取 Chrome Cookie、不得打开或修改 Chrome、不得复用主 Chrome 的 `web_session`。
